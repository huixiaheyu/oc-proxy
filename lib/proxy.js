...
export function registerChatRoutes(app, circuitBreaker) {
  ...
  app.post("/v1/chat/completions", async (req, res) => {
    ...
    try {
      const result = await withRetry(
        async () => {
          ...
          try {
            ...
            // 4xx 非 429 → 客户端错误，不重试、不计入熔断；抛错以便透传原始状态码
        ...
        },
        {
          retryableError: (err) => {
            ...
            // 测试请求（x-no-retry）：单次调用不重试，精确测量延迟
            if (req.headers?.["x-no-retry"]) return false;
      ...
      // 流式响应完整传输成功 → 记录熔断成功（中途失败会被 catch 捕获，不计成功）
      ...
      // 4xx（客户端错误）与 429 不计入熔断；只有真正的上游故障（5xx/连接错误）才记录
      const is4xx = err.status && err.status >= 400 && err.status < 500;
      if (circuitBreaker && err.status !== 429 && !is4xx) circuitBreaker.recordFailure(upstreamId);
      console.error("[oc-proxy] chat upstream error:", err.message);
      if (!res.headersSent) {
        // 4xx/429 透传原始状态码；否则统一 502
        const status = err.status && err.status >= 400 && err.status < 500 ? err.status : 502;
        const body = err.upstreamBody || `upstream error: ${err.message}`;
        res.status(status).json({ error: { message: body.slice(0, 500), type: "upstream_error" } });
      } else {
        res.end();
      }
    }
