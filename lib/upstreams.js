...
// 按 id 数组重新排序上游（ids 需包含全部自定义上游 id，顺序即最终展示顺序）
export function reorderUpstreams(ids) {
  if (!Array.isArray(ids)) throw new Error("ids 必须为数组");
  const { upstreams = [] } = readFile();
  if (ids.length !== upstreams.length) throw new Error("ids 数量与上游数不一致");
  const byId = new Map(upstreams.map((u) => [u.id, u]));
  const missing = ids.find((id) => !byId.has(id));
  if (missing) throw new Error(`上游不存在: ${missing}`);
  // 校验重复 id：重复会导致部分上游被静默丢弃
  if (new Set(ids).size !== ids.length) throw new Error("ids 中存在重复 id");
