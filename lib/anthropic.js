 ...
 */
export function registerMessagesRoute(app) {
  app.post("/v1/messages", async (req, res) => {
    ...
    openaiBody.model = modelId; // 上游只认不带前缀的模型名
    ...
    try {
      ...
      if (!upstreamRes.ok) {
        ...
        return res.status(upstreamRes.status).json(
          anthropicError(mapErrorType(upstreamRes.status), text.slice(0, 500) || `upstream HTTP ${upstreamRes.status}`)
        );
      }

      if (openaiBody.stream) {
        await streamUpstreamToAnthropic(upstreamRes, res, model, inputTokensEstimate);
      } else {
      ...
      if (!res.headersSent) {
        res.status(502).json(anthropicError("api_error", `upstream error: ${err.message}`));
      } else {
        res.end();
      }
    }
