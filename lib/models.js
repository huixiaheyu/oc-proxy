...
// 每个上游独立的缓存
// 用 upstream 对象引用作 key（而非 prefix 字符串），避免前缀变更后旧缓存残留 / 新 key 失效。
// 注意：upstreams 模块在增删改后会重建数组（reloadUpstreams），旧对象引用失效即自然丢弃，新对象重新拉取。
const caches = new Map(); // key: upstream 对象 → { data, expiresAt, fetching }
