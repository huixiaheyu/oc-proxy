...
// 每个上游独立的缓存，以 prefix 字符串为 key，避免对象引用泄漏
const caches = new Map(); // key: prefix 字符串 → { data, expiresAt, fetching }
      ...
      const result = await withRetry(
        ...
        },
        {
          ...
          onRetry: (attempt, delay, err) => {
            console.log(`[Models] ${upstream.name} retry=${attempt} delay=${delay}ms err=${err.message}`);
          },
        }
      );
